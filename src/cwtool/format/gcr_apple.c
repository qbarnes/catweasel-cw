 ...
 ****************************************************************************
 *
 * implements 6-and-2 encoding for 5.25" disks
 *
 ****************************************************************************
 ****************************************************************************/
...
gcr_apple_read_sector2(
	...
	*dsk_err = (struct disk_error) { };
	...
	/*
	 * UGLY: seems there is always one zero bit between sync and data
	 *       on 5.25" disks. would be better to strip it away only if
	 *       it exists
	 */
...
gcr_apple_read_sector(
	...
	result += format_compare2("data xor checksum: got 0x%02x, expected 0x%02x", data[DATA_SIZE - 1], gcr_apple_read_data_checksum(data, DATA_SIZE - 1));
