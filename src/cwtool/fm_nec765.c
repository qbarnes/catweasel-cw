...
fm_nec765_sector_shift(
	...
	{
	return (fm_get_sector_shift(fm_nec->rw.pshift, sector, CWTOOL_MAX_SECTOR));
	}
...
fm_nec765_read_sector2(
	...
	*dsk_err = (struct disk_error) { };
	if (fm_read_sync(ffo_l1, fm_nec->rw.sync_value1, fm_nec->rw.sync_value1) == -1) return (-1);
	...
	result = fm_read_sync(ffo_l1, fm_nec->rw.sync_value2, fm_nec->rw.sync_value3);
...
fm_nec765_read_sector(
	...
	result = fm_nec765_read_sector2(ffo_l1, fm_nec, &dsk_err, header, data);
	if (result == -1) return (-1);
...
fm_nec765_write_sector(
	...
	data_size = fm_nec765_sector_size(fm_nec, sector);
	fm_write_ushort_be(&header[4], fm_crc16(fm_nec->rw.crc16_init_value1, header, 4));
	disk_sector_write(data, dsk_sct);
	fm_write_ushort_be(&data[data_size], fm_crc16(fm_nec->rw.crc16_init_value2, data, data_size));
...
fm_nec765_write_track(
	...
	if (raw_write(&ffo_l1, ffo_l0, fmt->fm_nec.rw.bnd, fmt->fm_nec.wr.precomp, 2) == -1) return (0);
	return (1);
	}
...
fm_nec765_set_defaults(
	...
	const static struct fm_nec765	fm_nec =
		...
		.rw =
			...
			.bnd                =
				{
				BOUNDS(0x0800, 0x1b52, 0x2a00, 0),
				BOUNDS(0x2b00, 0x37a5, 0x4800, 1)
				}
			}
		};
	...
	fm_fill_sector_shift(fmt->fm_nec.rw.pshift, 0, CWTOOL_MAX_SECTOR, 0);
	}
...
fm_nec765_set_rw_option(
	...
	if (magic == MAGIC_SECTORS)
		{
		fm_fill_sector_shift(fmt->fm_nec.rw.pshift, fmt->fm_nec.rw.sectors, CWTOOL_MAX_SECTOR, 0);
		return (setvalue_uchar(&fmt->fm_nec.rw.sectors, val, 1, CWTOOL_MAX_SECTOR));
		}
	...
	if (magic == MAGIC_SECTOR_SIZES) return (fm_set_sector_size(fmt->fm_nec.rw.pshift, ofs, CWTOOL_MAX_SECTOR, val));
	debug_error_condition(magic != MAGIC_BOUNDS);
	return (setvalue_bounds(fmt->fm_nec.rw.bnd, val, ofs));
	}
...
static struct format_option		fm_nec765_read_options[] =
	{
	FORMAT_OPTION("ignore_sector_size",    MAGIC_IGNORE_SECTOR_SIZE,    1),
...
static struct format_option		fm_nec765_write_options[] =
	...
	FORMAT_OPTION("fill_value1",   MAGIC_FILL_VALUE1,   1),
	FORMAT_OPTION("fill_length2",  MAGIC_FILL_LENGTH2,  1),
	FORMAT_OPTION("fill_value2",   MAGIC_FILL_VALUE2,   1),
	FORMAT_OPTION("fill_length3",  MAGIC_FILL_LENGTH3,  1),
	FORMAT_OPTION("fill_value3",   MAGIC_FILL_VALUE3,   1),
	FORMAT_OPTION("fill_length4",  MAGIC_FILL_LENGTH4,  1),
	FORMAT_OPTION("fill_value4",   MAGIC_FILL_VALUE4,   1),
	FORMAT_OPTION("fill_length5",  MAGIC_FILL_LENGTH5,  1),
	FORMAT_OPTION("fill_value5",   MAGIC_FILL_VALUE5,   1),
	FORMAT_OPTION("fill_length6",  MAGIC_FILL_LENGTH6,  1),
	FORMAT_OPTION("fill_value6",   MAGIC_FILL_VALUE6,   1),
	FORMAT_OPTION("fill_length7",  MAGIC_FILL_LENGTH7,  1),
	FORMAT_OPTION("fill_value7",   MAGIC_FILL_VALUE7,   1),
	FORMAT_OPTION("precomp",       MAGIC_PRECOMP,       4),
	FORMAT_OPTION(NULL, 0, 0)
	};
...
static struct format_option		fm_nec765_rw_options[] =
	...
	FORMAT_OPTION("sync_value3",       MAGIC_SYNC_VALUE3,        1),
	FORMAT_OPTION("crc16_init_value1", MAGIC_CRC16_INIT_VALUE1,  1),
	FORMAT_OPTION("crc16_init_value2", MAGIC_CRC16_INIT_VALUE2,  1),
	FORMAT_OPTION("crc16_init_value3", MAGIC_CRC16_INIT_VALUE3,  1),
	FORMAT_OPTION("sector_sizes",      MAGIC_SECTOR_SIZES,      -1),
	FORMAT_OPTION("bounds",            MAGIC_BOUNDS,             6),
	FORMAT_OPTION(NULL, 0, 0)
	};
